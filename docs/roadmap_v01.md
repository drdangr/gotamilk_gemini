...
- [ ] Создать проект в Supabase, включить Realtime
  - [ ] Создать проект и получить `SUPABASE_URL` и `ANON_KEY`
...
- [ ] Подготовить проект на Vercel
  - [ ] Импорт репозитория, включить Preview Deployments
  - [ ] Добавить переменные окружения: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`
...
- [ ] Добавить зависимости и базовую инициализацию
  ...
  - [ ] Проверить доступность переменных окружения в dev/preview/prod

#### Этап 2 — Аутентификация и профиль
- [ ] Ввести контекст/провайдер сессии Supabase
...
- [ ] Экран входа/редирект
  ...
  - [ ] Обработка redirect back от провайдера
- [ ] Синхронизация профиля пользователя
  - [ ] Создание/обновление записи в `profiles` при первом логине
  - [ ] Заменить `CURRENT_USER` на данные из сессии

#### Этап 3 — Схема БД и RLS
- [ ] Создать таблицы
  - [ ] `profiles`, `lists`, `list_members`, `list_items`, `products`, `aliases`
- [ ] Связи, индексы, ограничения
  - [ ] FK на владельца списка и принадлежность элементов списку
  - [ ] Индексы: `list_items.list_id`, `list_members(list_id,user_id)`
- [ ] Включить RLS и политики
  - [ ] Политики чтения для участников
  - [ ] Политики записи для ролей `owner`/`editor`
- [ ] Базовый сидинговый набор (опционально для dev)

#### Этап 4 — CRUD списков и элементов
- [ ] Экран/меню выбора активного списка
  - [ ] Создание дефолтного «My list» при первом входе
...
#### Этап 5 — Realtime
- [ ] Подписки на `list_items`
  - [ ] Обработка insert/update/delete для активного списка
  - [ ] Дедупликация/идемпотентность апдейтов в клиенте
